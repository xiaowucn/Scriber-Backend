...
# pre
if [[ ${SCRIBER_SZSE_IPO_PROD} != "True" ]]; then
  exit 0
fi
...
run rsync -avz --safe-links --delete \
  ...
  ./ /data/scriber_szse_ipo_prod/code_src/Scriber-Backend/
...
# restart
run docker exec -i scriber_szse_ipo_prod_web ./docker/deploy_upgrade.sh

# mm
bash /data/ci/fitout/autodoc/send_mm_msg.sh \
  http://mm.paodingai.com/hooks/xffd4wkndpnjubqd9z9puzoxaa \
  scriber \[Scriber深交所IPO数据抽取填报内部正式环境\(46027\)\]\(http://100.64.0.5:46027/\)已更新\:\ \`前端\:${GO_SCRIBER_FRONT_COMMIT}\(${GO_SCRIBER_FRONT_BRANCH}\)\`\ \`后端\:${GO_REVISION_SCRIBER_BACKEND:0:8}\(${GO_MATERIAL_BRANCH_SCRIBER_BACKEND}\)\`
