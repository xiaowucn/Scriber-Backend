...
# pre
if [[ ${scriber_ccxi_market_dev} == "True" ]]; then
  echo "runing ..."
else
  echo "passing ..."
  exit 0
...
# frontend
run rsync -avz --progress --exclude=.git --delete ../dist_ccxi_contract/dist_ccxi_contract/scriber_front/dist/ ./remarkable/static/
...
run rsync -avz --progress --delete --safe-links \
  ...
  ./ /data/scriber_ccxi_market_dev/code_src/Scriber-Backend/
...
# restart
run docker exec -i scriber_ccxi_market_dev_web ./docker/deploy_upgrade.sh

# mm
bash /data/ci/fitout/autodoc/send_mm_msg.sh \
  http://mm.paodingai.com/hooks/xffd4wkndpnjubqd9z9puzoxaa \
  scriber \[Scriber中诚信市场部测试环境\(22113\)\]\(http://100.64.0.3:22113/\)已更新\:\ \`前端\:${GO_SCRIBER_FRONT_COMMIT}\(${GO_SCRIBER_FRONT_BRANCH}\)\`\ \`后端\:${GO_REVISION_SCRIBER_BACKEND_MASTER:0:8}\(${GO_MATERIAL_BRANCH_SCRIBER_BACKEND_MASTER}\)\`
