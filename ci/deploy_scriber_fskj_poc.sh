...
# backend
run rsync -avz --progress --safe-links --delete --exclude=/data/export_answer_data --exclude=/data/files --exclude=/data/prompter --exclude=/data/training_cache --exclude=/data/tbl_headers --exclude=/healthcheck.py --exclude=/i18n/**.po --exclude=/i18n/**.pot ./ /data2/scriber_fskj_poc/code_src/Scriber-Backend
...
# restart
run docker exec -i scriber_fskj_poc_web ./docker/deploy_upgrade.sh

# mm
bash /data/ci/fitout/autodoc/send_mm_msg.sh \
  http://mm.paodingai.com/hooks/xffd4wkndpnjubqd9z9puzoxaa \
  scriber \[Scriber凡声科技poc测试环境\]\(http://100.64.0.10:12105\)已更新\:\ \`前端\:${GO_SCRIBER_FRONT_COMMIT}\(${GO_SCRIBER_FRONT_BRANCH}\)\`\ \`后端\:${GO_REVISION_SCRIBER_BACKEND:0:8}\(${GO_MATERIAL_BRANCH_SCRIBER_BACKEND}\)\`
