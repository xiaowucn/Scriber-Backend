...
# pre
if [[ ${SCRIBER_SZSE_ANNUAL_AUDIT_DEMO} == "True" ]]; then
  echo "runing ..."
else
  echo "passing ..."
  exit 0
...
run rsync -avz --safe-links --delete \
  ...
  ./ /data/scriber_szse_annual_audit_demo/code_src/Scriber-Backend/
...
# restart
run docker exec -i scriber_szse_annual_audit_demo_web ./docker/deploy_upgrade.sh

# mm
bash /data/ci/fitout/autodoc/send_mm_msg.sh \
  http://mm.paodingai.com/hooks/xffd4wkndpnjubqd9z9puzoxaa \
  scriber \[Scriber深交所年报审核测试环境\(44556\)\]\(http://100.64.0.5:44556/\)已更新\:\ \`前端\:${GO_SCRIBER_FRONT_COMMIT}\(${GO_SCRIBER_FRONT_BRANCH}\)\`\ \`后端\:${GO_REVISION_SCRIBER_BACKEND_MASTER:0:8}\(${GO_MATERIAL_BRANCH_SCRIBER_BACKEND_MASTER}\)\`
