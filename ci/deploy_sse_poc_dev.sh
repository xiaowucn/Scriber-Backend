...
# pre
if [[ ${SCRIBER_SSE_POC_DEV} == "True" ]]; then
  echo "runing ..."
else
  echo "passing ..."
  exit 0
...
run rsync -avz --progress --delete --safe-links \
  ...
  ./ /data/scriber_sse_poc_dev/code_src/Scriber-Backend/
...
# restart
run docker exec -i scriber_sse_poc_dev_web ./docker/deploy_upgrade.sh

# mm
bash /data/ci/fitout/autodoc/send_mm_msg.sh \
  http://mm.paodingai.com/hooks/xffd4wkndpnjubqd9z9puzoxaa \
  scriber \[Scriber上交所合规检测POC内部测试环境\(44026\)\]\(http://100.64.0.5:44026/\)已更新\:\ \`前端\:${GO_SCRIBER_FRONT_COMMIT}\(${GO_SCRIBER_FRONT_BRANCH}\)\`\ \`后端\:${GO_REVISION_SCRIBER_BACKEND_MASTER:0:8}\(${GO_MATERIAL_BRANCH_SCRIBER_BACKEND_MASTER}\)\`
